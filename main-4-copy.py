   ...
   #     for row in record:
    ...
    def capture2(self,ui,Dialog):
        while(True):
            ...
            faces = faceDetector.detectMultiScale(erosion, 1.3, 5)
            for (x,y,w,h) in faces:
                cv2.rectangle(img,(x,y),(x+w,y+h),(255,0,0),2)
                id=rec.predict(gray[y:y+h,x:x+w])
                print(id)
                profile=self.inorup1(id[0])
                print(profile[0])
                profile = profile[0]
                if(profile!=None):
                    cv2.putText(img, profile, (x,y+h+30), font, fontscale, fontcolor)
                    #cont = 1
                    #cap.release()
                    #ui.setupUi(Dialog)
        ...
        cv2.destroyAllWindows()
            #self.capture.setAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
            
    def inorup(self,id):
        cur=conn.cursor()
        query2 = "SELECT * FROM user WHERE id=" +  str(id)
        result2 = cur.execute(query2)
        record = cur.fetchall()
        name1 = ""
    ...
    def inorup1(self,id):
        cur=conn.cursor()
        query2 = "SELECT name FROM user WHERE id=" + str(id)
        result2 = cur.execute(query2)
        record = cur.fetchall()
        isRecord = 0
        name = ""
    ...
    def show_next(self,ui,Dialog):
        ui.setupUi(Dialog)
        ui.capture2(ui,Dialog)
        #ui.load_data()
    def setupUi1(self, Dialog):
        ...
        self.frame_3.setGeometry(QtCore.QRect(0, 0, 1271, 561))
    ...
    def setupUi(self, Dialog):
        ...
        #self.pushButton_2.clicked.connect(self.skip_data)
